...
class CSVBuffer:
    ...
    def sort_by_column(self, col_idx: int, ascending: bool = True):
        ...
        try:
            # Try to detect data type for smart sorting
            sample_values = [row[col_idx] for row in self.data[:10] if col_idx < len(row) and row[col_idx].strip()]
            
            is_numeric = True
            for val in sample_values[:5]:  # Check first 5 non-empty values
                try:
                    float(val)
                except ValueError:
                    is_numeric = False
            ...
            if is_numeric:
                # Numeric sort
                self.data.sort(key=lambda row: float(row[col_idx]) if col_idx < len(row) and row[col_idx].strip() else float('inf'), reverse=not ascending)
            else:
                # String sort
                self.data.sort(key=lambda row: row[col_idx].lower() if col_idx < len(row) else "", reverse=not ascending)
    ...
    def load_from_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            if path.suffix.lower() == '.csv':
                ...
                # XLSX loading with sheet selection
                ...
                # Update file path to include sheet name for saving
                base_name = path.stem
            ...
            self.auto_resize_columns()  # Auto-resize on load
            return True
...
class VimCSVEditor:
    ...
    def tab_complete_command(self):
        ...
        commands = ['write', 'quit', 'help', 'new', 'edit', 'goto', 'sort', 'find', 'replace']
...
ENHANCED FEATURES:
------------------
• Number prefixes:    5j (move down 5 rows), 3dd (delete 3 rows)
...
[count]o     Insert [count] rows below
[count]d     Delete [count] rows
...
"""
    ...
    def draw_screen(self, stdscr):
        ...
        help_y = height - 2
        if self.mode == 'NORMAL':
            help_text = "hjkl:move i:edit I:fullscreen v:visual y:copy p:paste /:search s:sort =:resize F1:help q:quit"
